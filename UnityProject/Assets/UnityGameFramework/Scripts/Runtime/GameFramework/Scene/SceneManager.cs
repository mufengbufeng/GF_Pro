...
namespace GameFramework.Scene
{
    ...
    {
        private readonly List<string> m_LoadedSceneAssetNames;  // 用于记录已加载的场景资源名称，避免重复加载。
        private readonly List<string> m_LoadingSceneAssetNames; // 用于记录正在加载的场景资源名称，避免重复加载。
        private readonly List<string> m_UnloadingSceneAssetNames;   // 用于记录正在卸载的场景资源名称，避免重复卸载。
        private readonly LoadSceneCallbacks m_LoadSceneCallbacks;   // 用于加载场景的回调函数集。
        private readonly UnloadSceneCallbacks m_UnloadSceneCallbacks;   // 用于卸载场景的回调函数集。
        ...
        private SceneHandle GetSceneHandle(string sceneAssetName, LoadSceneMode sceneMode = LoadSceneMode.Single, bool suspendLoad = false, int priority = 100, string packageName = "")
        {
            ...
            {
                // LocalPhysicsMode
                // return YooAssets.LoadSceneAsync(sceneAssetName, sceneMode, suspendLoad, priority: (uint)priority);
                return YooAssets.LoadSceneAsync(sceneAssetName, sceneMode, suspendLoad: suspendLoad, priority: (uint)priority);
            }

            var package = YooAssets.GetPackage(packageName);
            // return package.LoadSceneAsync(sceneAssetName, sceneMode, suspendLoad, (uint)priority);
            return package.LoadSceneAsync(sceneAssetName, sceneMode, priority: (uint)priority);
        }
        ...
        private void UnloadScene(string sceneAssetName, UnloadSceneCallbacks unloadSceneCallbacks, object userData = null, string packageName = "")
        {
            ...
            // {
            //     if (m_UnloadingSceneAssetNames.Contains(item: sceneAssetName))
            //     {
            //         m_UnloadingSceneAssetNames.Remove(sceneAssetName);
            //     }
            //     return;
