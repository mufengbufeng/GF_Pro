...
namespace GameFramework.Resource
{
    ...
    internal sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        public void Initialize()
        {
            ...
            Log.Info("ResourceManager initialized successfully.");
        }
        ...
        public async UniTask<InitializationOperation> InitPackage(string packageName)
        {
            ...
            if (playMode == EPlayMode.EditorSimulateMode)
            {
                ...
                initializationOperation = InitEditorSimulateMode(packageName, package);
            }
            ...
            if (playMode == EPlayMode.OfflinePlayMode)
            {
                ...
                // initializationOperation = package.InitializeAsync(createParameters);
                initializationOperation = InitOfflinePlayMode(packageName, package);
            ...
            if (playMode == EPlayMode.HostPlayMode)
            {
                ...
                // initializationOperation = package.InitializeAsync(createParameters);
                initializationOperation = InitHostPlayMode(packageName, package);
            }
            ...
            if (playMode == EPlayMode.WebPlayMode)
            {
                ...
                // createParameters.RemoteServices = new RemoteServices(defaultHostServer, fallbackHostServer);
                // initializationOperation = package.InitializeAsync(createParameters);
                initializationOperation = InitWebPlayMode(packageName, package);
            }

            await initializationOperation.ToUniTask();
            if (initializationOperation.Status == EOperationStatus.Failed)
            {
                Log.Error($"Init resource package failed : {initializationOperation.Error}");
            ...
            Log.Info($"Init resource package version : {initializationOperation}");

            return initializationOperation;
        }

        private UpdatePackageManifestOperation UpdatePackageManifest(string packageName, string packageVersion)
        {
        ...
        public InitializationOperation InitEditorSimulateMode(string packageName, ResourcePackage package)
        {
            ...
            Log.Info($"EditorSimulateModeParameters : {initParameters}");
            return initOperation;
        }

        public InitializationOperation InitOfflinePlayMode(string packageName, ResourcePackage package)
        {
            var buildinFileSystemParams = FileSystemParameters.CreateDefaultBuildinFileSystemParameters();
            var initParameters = new OfflinePlayModeParameters();
            initParameters.BuildinFileSystemParameters = buildinFileSystemParams;
            var initOperation = package.InitializeAsync(initParameters);
            Log.Info($"OfflinePlayModeParameters : {initParameters}");
            return initOperation;
        }

        public InitializationOperation InitHostPlayMode(string packageName, ResourcePackage package)
        {
            ...
            var initOperation = package.InitializeAsync(initParameters);
            Log.Info($"HostPlayModeParameters : {initParameters}");
            return initOperation;
        }

        public InitializationOperation InitWebPlayMode(string packageName, ResourcePackage package)
        {
            IRemoteServices remoteServices = new RemoteServices(HostServerURL, FallbackHostServerURL);
            ...
            var initOperation = package.InitializeAsync(initParameters);
            Log.Info($"WebPlayModeParameters : {initParameters}");
            return initOperation;
        }
